...
const Recipients = () => {
  ...
  useEffect(() => {
    async function getData() {
      const recipients = await App.w3payContract.filters.recipeints(App.address)
      const recipentsData = await App.w3payContract.queryFilter(recipients);
  ...
  const addRecipient = async () => {
    try {
      const tx = await App.w3payContract.addRecipient(recipientAddress, recipientName);
      await tx.wait();
